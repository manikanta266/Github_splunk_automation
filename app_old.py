...
def create_jenkins_job_1():
    ...
    index_name = data.get('index_name', 'default-index')
    repo_url = data.get('repo_url', 'https://github.com/example/example.git')
    repo_name = repo_url.rstrip('/').split('/')[-1].replace('.git', '')
    email_recipient = data.get('email', 'default@example.com')
